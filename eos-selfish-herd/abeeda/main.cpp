...
int main(int argc, char *argv[])
{
    ...
    double swarmden[totalGenerations];
    double hdaa_p[totalGenerations];
	...
	for (int update = 1; update <= totalGenerations; ++update)
    {
		...
		for(int i = 0; i < populationSize; ++i)
        {
            game->executeGame(swarmAgents, predatorAgents[i], NULL, false, startingDist, predatorVisionRange, predatorVisionAngle, killDelay, numPredators, swarmdensity, hdaa_no);
        ...
        swarmdensity=swarmdensity/swarmSize;

        cout<<swarmdensity<<endl;
        cout<<hdaa_no<<endl;

        swarmden[update]=swarmdensity;
        hdaa_p[update]=hdaa_no;
        ...
        if (make_interval_video)
        {
            ...
            {
                string bestString = game->executeGame(swarmAgents, bestPredatorAgent, NULL, true, startingDist, predatorVisionRange, predatorVisionAngle, killDelay, numPredators, swarmdensity, hdaa_no);
    ...
    for (int i = 0; i < swarmLOD.size(); ++i)
    {
        ...
        game->executeGame(cloneSwarm, predatorLOD[i], LOD, false, startingDist, predatorVisionRange, predatorVisionAngle, killDelay, numPredators, swarmdensity, hdaa_no);
...
//************************************************************************************************************
   //********************************************************************************************************************
     ofstream file3("swarmdensity.csv");

    if (file3.is_open()) {
        // Assuming you are writing some x and y data points
        for (int i = 0; i < totalGenerations; ++i) {
            double x4 = generations[i]; 
            double y4 = swarmden[i]; 
            file3 << x4 << "," << y4 << "\n";
        }
        file3.close();
    } else {
    ...
    //********************************************************************************************************************
     ofstream file4("HDAA_counter.csv");

    if (file4.is_open()) {
        // Assuming you are writing some x and y data points
        for (int i = 0; i < totalGenerations; ++i) {
            double x5 = generations[i]; 
            double y5 = hdaa_p[i]; 
            file4 << x5 << "," << y5 << "\n";
        }
        file4.close();
...
string findBestRun(vector<tAgent*> swarmAgents, tAgent *predatorAgent)
{
    ...
    {
        reportString = game->executeGame(swarmAgents, predatorAgent, NULL, true, startingDist, predatorVisionRange, predatorVisionAngle, killDelay, numPredators, swarmdensity, hdaa_no);
